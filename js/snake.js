////Snake développer en ECMAScript 6 ////
//Variables globales
var speed = 100;
var size = 10;
...
class Player extends objet {
	...
	increaseLevel(){
		speed = speed /2;
		this.level++;
		var levelBox = document.getElementById('levelValue');
		levelBox.textContent = this.level;
	}
	meter(){
		if(this.following == null){
...
/// Fin du jeu on rénitialise les variables, génére un nouveau snake et de la nouvelle nourriture et on affiche "perdu".
function endGame(){
...
function main() {
	...
	if(player.collision(food)){
		player.nbFood++; // On incrément le nombre de nourriture que le jouer a trouvé
		if(player.nbFood >= 5){ // On augmente le level
			player.increaseLevel();
			levelBox.style.display = "block";
			player.nbFood = 0; // On remet a 0 pour pouvoir calculer le prochain niveau.
		}
		player.increaseScore();
		food.position();
		player.meter(); // biensur on oublie pas d'agrandir le snake.
	}
}
